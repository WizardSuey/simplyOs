...
// Структура для отдельной записи GDT (не для всей таблицы)
typedef struct {
    uint32_t base;
    uint32_t limit;
    uint16_t flags;
...
// Один дескриптор в GDT (8 байт, как в памяти)
typedef struct {
    uint16_t limit_low;      // Младшие 16 бит лимита
    uint16_t base_low;       // Младшие 16 бит базы
    uint8_t  base_middle;    // Следующие 8 бит базы
    uint8_t  access;         // Флаги доступа (type, DPL, P)
    uint8_t  granularity;    // Старшие 4 бита лимита + флаги (G, D/B, L, AVL)
